...
namespace XHX.Common
{
    public class LocalService
    {
        ...
        #region ShopUser
        #region Position
        public DataSet PositionSave(int positionCode, string positionName, string remark, string userId)
        {
            string sql = string.Format("Exec up_SU_Position_S '{0}','{1}','{2}','{3}'", positionCode, positionName, userId, remark);
            DataSet ds = CommonHandler.query(sql);
            return ds;
        }
        public DataSet PositionSearch(int positionCode, string positionName)
        {
            string sql = string.Format("Exec up_SU_Position_R '{0}','{1}'", positionCode, positionName);
            DataSet ds = CommonHandler.query(sql);
            return ds;
        }
        #endregion
        #region Area
        public DataSet AreaSearch(string projectCode, string areaName, string areaCode)
        {
            string sql = string.Format("Exec up_SU_Area_R '{0}','{1}','{2}'", projectCode, areaCode, areaName);
            return CommonHandler.query(sql);
        }
        public DataSet AreaSave(string projectCode, string areaName, string areaCode, string userID)
        {
            string sql = string.Format("Exec up_SU_Area_S '{0}','{1}','{2}','{3}','{4}','{5}'",
                    projectCode, areaCode, areaName, "", "", userID);
            return CommonHandler.query(sql);
        }
        #endregion
        #region Shop
        public DataSet ShopSearch(string projectCode, string shopCode, string shopName)
        {
            string sql = string.Format("Exec [up_SU_Shop_R] '{0}','{1}','{2}'", projectCode, shopCode, shopName);
            return CommonHandler.query(sql);
        }
        public DataSet ShopSave(string projectCode, string shopCode, string shopName, string areaCode, string userID)
        {
            string sql = string.Format("Exec up_SU_Shop_S '{0}','{1}','{2}','{3}','{4}'",
                                      projectCode, shopCode, shopName, userID, areaCode);
            return CommonHandler.query(sql);
        }
        public DataSet ShopDelete(string projectCode, string shopCode)
        {
            string sql = string.Format("Exec up_SU_Shop_D '{0}','{1}'",
                                          projectCode, shopCode);
            return CommonHandler.query(sql);
        }
        #endregion
        #region Projects
        public void ProjectSave(char statusType, string projectCode, string year, string quarter)
        {
            string sql = String.Empty;
            if (statusType == 'I')
            {
                sql = string.Format("EXEC up_SU_Projects_S '{0}','{1}','{2}','{3}','{4}'", year + quarter,
                    year + quarter, "sysadmin", year, quarter);
            }
            else if (statusType == 'U')
            {
                sql = string.Format("EXEC up_SU_Projects_S '{0}','{1}','{2}','{3}','{4}'", projectCode,
                    projectCode, "sysadmin", year, quarter);
            }
            CommonHandler.query(sql);
        }
        public DataSet ProjectSearch()
        {
            string sql = string.Format("EXEC up_SU_Projects_R");
            DataSet ds = CommonHandler.query(sql);

            return ds;
        }
        public void CopyData(string projectCode)
        {
            string sql = string.Format("EXEC up_AddNewSeasonData '{0}'", projectCode);
            DataSet ds = CommonHandler.query(sql);

        }
        #endregion
        #region LossRate
        public DataSet LossRateMstSearch(string projectCode, string shopCode)
        {
            string sql = string.Format("EXEC up_SU_LeaveRatioMst_R '{0}','{1}'", projectCode, shopCode);
            return CommonHandler.query(sql);
        }
        public DataSet LossRateDtlSearch(string projectCode, string shopCode)
        {
            string sql = string.Format("EXEC up_SU_LeaveRatioDtl_R '{0}','{1}'", projectCode, shopCode);
            return CommonHandler.query(sql);
        }
        #endregion
        #region PadToDB
        public void ShopUserInfoUpload(List<String> dataList)
        {
            foreach (String data in dataList)
            {
                try
                {
                    ...
                    string pinYinName = properties[3];
                    string userPosition = properties[4];
                    string ahth = properties[5];
                    string inuserId = properties[6];
                    string inDateTime = properties[7];
                    string picNameList = properties[8];

                    ShopUserInfoSave(projectCode, shopCode, userName, pinYinName, userPosition
                        , ahth, inuserId, Convert.ToDateTime(inDateTime), picNameList);
                }
                catch (Exception)
                {
        ...
        #endregion
        #region ShopUserInfo
        public void ShopUserInfoSave(string projectCode, string shopCode, string userName, string pinYinName,
            string userPosition, string authenticate, string inUserId, DateTime inDateTime, string picNameList)
        {
            string sql = string.Format("Exec up_SU_ShopUserMng_S '{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}'",
                projectCode,shopCode,userName,pinYinName,userPosition,authenticate,inUserId,picNameList,inDateTime);
            CommonHandler.query(sql);
        }
        public DataSet ShopUserInfoSearch(string projectCode, string shopCode)
        {
            string sql = string.Format("Exec up_SU_ShopUserMng_R '{0}','{1}'",
                projectCode, shopCode);
            return CommonHandler.query(sql);
        }
        #endregion
        #region HiddenCode
        public DataSet AuthSearch()
        {
            string sql = string.Format("Exec up_SU_Authentic_R");
            return CommonHandler.query(sql);
        }
        #endregion
        #endregion
